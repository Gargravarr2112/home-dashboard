...
with open('weather.json', 'r') as configFile:
	try:
		...
		logger.error("Failed to load weather.json:")
		logger.error(e)
...
def getWeatherType(typeCode):
	...
	isDay = hour > DAY_START and hour < DAY_END
	if isDay:
		weatherTime = 'day'
	else:
		weatherTime = 'night'
	weatherImage = "img/{0}.png".format(WEATHER_IMAGE_KEY[typeCode].replace('$t', weatherTime))
